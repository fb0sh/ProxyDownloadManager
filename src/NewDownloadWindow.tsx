...
async function readClipboardUrl(): Promise<string | null> {
  try {
    ...
    // Clipboard read failed (permission, unavailable, or empty)
  }
  return null;
}
...
export default function NewDownloadWindow() {
  ...
  useEffect(() => {
    ...
    if (initial) {
      // URL passed from main window (clipboard detection there)
      setUrl(initial);
      if (extractFilename(initial)) {
        setFilename(extractFilename(initial));
        setAutoFilled(true);
      }
    } else {
      // No URL from parent — read clipboard directly (cross-platform)
      readClipboardUrl().then((clipUrl) => {
        if (clipUrl) {
          setUrl(clipUrl);
          if (extractFilename(clipUrl)) {
            setFilename(extractFilename(clipUrl));
            setAutoFilled(true);
          }
        }
      });
  ...
  // Sync backend settings into zustand store and local state
  useEffect(() => {
    if (loadedSettings) {
