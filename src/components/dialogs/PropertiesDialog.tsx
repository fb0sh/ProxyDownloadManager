...
export default function PropertiesDialog({ id, onClose }: PropertiesDialogProps) {
  ...
  return (
    <Dialog title="Details" onClose={onClose} width="large">
      <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
        ...
        <div style={{ padding: "12px 20px" }}>
          <div style={sectionStyle}>
            ...
            <InfoRow label="Size" value={formatBytes(item.total_size)} />
            <InfoRow label="Downloaded" value={`${formatBytes(item.downloaded)} (${progress > 0 ? Math.round(progress) + "%" : "0%"})`} />
            <InfoRow label="Save Path" value={item.save_path || "—"} />
            <InfoRow label="Created" value={item.created_at} last />
          </div>
        </div>
        ...
        <div style={{ padding: "0 20px 12px" }}>
          <div style={sectionStyle}>
            ...
            <InfoRow label="Resumable" value={item.resumable === true ? "Yes" : item.resumable === false ? "No" : "Unknown"} />
            <InfoRow label="Last Try" value={item.last_try || "—"} last />
          </div>
        </div>
        ...
        <div style={{ padding: "0 20px 16px" }}>
          <div style={sectionStyle}>
            ...
            <InfoRow label="Proxy" value={item.proxy_name || "None"} last />
          </div>
        </div>
      </div>
    </Dialog>
